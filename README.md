...
문제를 정의하고, 그걸 반복해서 해결할 수 있는 수단을 만듭니다.

한 번 손으로 하면 다음엔 자동으로 돌아가야 한다고 생각해요. AI가 잘못된 수치를 쓰는 걸 발견하면 "이번만 고치자"가 아니라, 그 오류가 왜 생기는지 패턴을 찾고 시스템이 스스로 감지하고 교정하게 만듭니다. 문제 하나를 깊게 정의해두면 나중에 같은 문제가 다시 생겨도 사람 손이 필요 없어지니까요.

지금은 매일 오후 3시 40분, 제 컴퓨터가 알아서 주식 시장을 분석하고, 글을 쓰고, 오류를 점검하고, 블로그에 올립니다. 처음엔 하나하나 손으로 하던 일들이에요.
